var Graph = {
  ...
  returnDecimal: function(num) {
    num = num + "";
    var toReturn;
    // Handle exponential numbers.
    if(num.indexOf("e") > -1){
        var tempArr = num.split("e");
        var decPos = tempArr[0].indexOf(".");
        if(decPos == -1){
            decPos = 0;
        }
        else{
            toReturn = tempArr[0].replace(".","");
        }
        for(i = 0; i<parseInt(tempArr[1])-decPos;i++){
            toReturn = toReturn + "" + "0";
        }
        return toReturn;
    }
    ...
    if (isFinite(yVal)) {
      var path = "M20 " + (Graph.xAxisPosition - Graph.returnDecimal(parseFloat(yVal * this.widthy))) + " ";
    } else {
      path = "M20 " + (Graph.xAxisPosition - Graph.returnDecimal(parseFloat(parseInt($('#maxY').val()) * this.widthy) * 20)) + " ";
    }
